...
| Step | Tool |
|---|---|
| Rig and guides | `tools/blender_rig.py` (headless: `blender -b --factory-startup -P tools/blender_rig.py -- pose.json out_dir [--body male|female]`) writes `depth.png`, `shaded.png`, `openpose.json` (COCO-18 plus a `sword` hilt->tip line) and `openpose.png`. Hand posing (Ian) happens in the **Rigify** files `art_src/mc/rig/farroad_rigify_male.blend` / `farroad_rigify_female.blend` (built by `tools/rigify_rig.py`; IK/FK limbs, foot roll, torso/chest/hips/head controls; Farroad sidebar tab saves `"format": "rigify"` poses; render one headless with `blender -b art_src/mc/rig/farroad_rigify_<body>.blend -P tools/rigify_rig.py -- --render pose.json out_dir`). `blender_rig.py` (IK targets) stays as the automatic fitter's rig; its skeleton output has the same format. **Two body profiles** (`--body male|female`, or `"body"` in the pose JSON; default male), measured off each master: total height 3.9, male neck/shoulder/hip/knee at 0.77/0.71/0.47/0.23 of it, female 0.745/0.705/0.46/0.21, with a bigger head and narrower shoulders. Render the male guide for the male master and the female guide for the female master. Read proportions from the module after `set_body()`; don't copy constants. |
